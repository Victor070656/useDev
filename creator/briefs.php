...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600">Find projects that match your skills and interests</p>
    </div>
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="type" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <option value="fixed" <?= $projectType === 'fixed' ? 'selected' : '' ?>>Fixed Price</option>
                    <option value="hourly" <?= $projectType === 'hourly' ? 'selected' : '' ?>>Hourly</option>
                </select>
            </div>
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full px-6 py-3 bg-purple-600 text-white rounded-xl font-semibold hover:bg-purple-700 transition">
                    Apply Filters
                </button>
            </div>
        </form>
    ...
    <?php else: ?>
        <div class="space-y-4 mb-8">
            <p class="text-gray-600"><?= $total ?> project<?= $total !== 1 ? 's' : '' ?> found</p>
            ...
            <?php foreach ($briefs as $brief): ?>
                <div class="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                        ...
                        <a href="brief-detail.php?id=<?= $brief['id'] ?>" class="px-6 py-2 bg-purple-600 text-white rounded-full font-semibold hover:bg-purple-700 transition">
        ...
        <?php if ($pagination['total_pages'] > 1): ?>
            <div class="flex items-center justify-center space-x-2">
                <?php if ($pagination['has_prev']): ?>
                    <a href="?type=<?= $projectType ?>&search=<?= urlencode($search) ?>&page=<?= $pagination['prev_page'] ?>" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                        Previous
                    </a>
                <?php endif; ?>
                ...
                <?php if ($pagination['has_next']): ?>
                    <a href="?type=<?= $projectType ?>&search=<?= urlencode($search) ?>&page=<?= $pagination['next_page'] ?>" class="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                        Next
                    </a>
                <?php endif; ?>
            </div>
