...
$stmt = db_prepare("SELECT total_earned FROM creator_profiles WHERE user_id = ?");
...
<div class="max-w-7xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-gray-900 mb-8">Earnings & Payouts</h1>

    <!-- Stats Cards -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white rounded-2xl shadow-lg p-6">
            <h3 class="text-sm font-medium text-gray-500 mb-2">Total Earnings</h3>
            <p class="text-3xl font-bold" style="background: linear-gradient(135deg, #240046 0%, #7103a0 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-xl p-6 mb-8">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600">You have <?= format_money($availableForPayout) ?> available for withdrawal</p>
                </div>
    ...
    <?php else: ?>
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-8">
            <p class="text-gray-600 text-center">Minimum payout amount is <?= format_money(MINIMUM_PAYOUT) ?>. Keep working to reach the threshold!</p>
        </div>
    <?php endif; ?>
